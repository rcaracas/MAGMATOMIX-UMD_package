...
def read_outcar(FileName,InitialStep,SkipStep):
    ...
    with open(FileName,'r') as ff:
        while True:
            ...
            if(switch==False):
                if (len(entry)==3 and entry[0]=='POTIM'):
                    TimeStep=float(entry[2])
                if len(entry) >= 9 and entry[4] == '=-STRESS':
                    switch = True
                elif len(entry) >= 9 and entry[3] == '=-STRESS':
                    switch = True
                else:
                    continue
...
#                        MyCrystal.internalenergy=float(entry[3])  #this leads only part of the internal energy, one should add the kinetic energy of ions
                    ...
                    if MyCrystal.lambda_ThermoInt < 1.0:
                        ...
                        MyCrystal.internalenergy = float(entry[4])
                elif entry[0] == 'energy' and entry[2] == 'entropy=':#reading Kohn-Sham energy, contains all the electronic energy without the electronic entropy
                                #MyCrystal.internalenergy=float(entry[3])  #this leads only part of the internal energy, one should add the kinetic energy of ions
                                #the variance of {this energy + kinetic energy of ions}  yields Cv
                    flagscalee += 1
                    MyCrystal.ML = 0
