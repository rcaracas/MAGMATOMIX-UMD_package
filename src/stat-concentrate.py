...
def molecularmass(formula,chem_list):
    ...
    for ielem in range(len(elements)):
        ...
        if elements[ielem] in chem_list or '_' in chem_list:
            massA = massA + float(mass) * float(indexes[ielem])
            occurs += float(indexes[ielem])  # accumulate over all matching elements
...
def concatstatfile(FileName,population,iround,clusterlength,lifetime,chemistry):
    chem_list = [c.strip() for c in chemistry.split(',')]
    ch = 'every element' if '_' in chem_list else '+'.join(chem_list)
    ...
    while True:
        ...
        line=line.strip()
        entry=line.split()
        molec_size = int(entry[3])                  # atom count is in column 3
        flagcluster = -1        #flag to check the existence of the cluster
        totaltime = totaltime + float(entry[1])
        if molec_size <= clusterlength:             # cluster is in the gas
                                                    # clusters smaller than clusterlength are considered vapor
            totaltimevapor = totaltimevapor + float(entry[1])
            (massM,massA,occurs,totalatomsinmolec)=molecularmass(entry[0],chem_list)
            totalmassvapor = totalmassvapor + massM * float(entry[1])               #defined as total mass of the vapor molecules x lifetimes
            totalatomsinvapor = totalatomsinvapor + totalatomsinmolec * float(entry[1])
            if int(occurs) > 0:
                totaltimemoleculeswithchemistryinvap = totaltimemoleculeswithchemistryinvap + float(entry[1])           # defined as sum of lifetimes of gas.molecules.with.chemistry
                totalmasscheminvap = totalmasscheminvap + massA * float(entry[1])       #defined as no.of chemistry x lifetime x mass
                totaltimecheminvap = totaltimecheminvap + occurs * float(entry[1])      #defined as no.of chemistry x lifetime
            for ii in range(len(population)):
                if population[ii][0] == entry[0] and population[ii][2] == 'V':   #cluster already exists in vapor
                    population[ii][iround+3] = entry[1]
                    flagcluster = 0
            if flagcluster == -1:
                population.append(['' for _ in range(len(population[0]))])
                population[len(population)-1][0] = entry[0]
                population[len(population)-1][1] = str(molec_size)
                population[len(population)-1][2] = 'V'
                population[len(population)-1][iround+3] = entry[1]
        else:                                   #cluster is in the liquid
            totaltimeliquid = totaltimeliquid + float(entry[1])
            (massM,massA,occurs,totalatomsinmolec)=molecularmass(entry[0],chem_list)
            totalatomsinliquid = totalatomsinliquid + totalatomsinmolec * float(entry[1])
            if int(occurs) > 0:
                totaltimemoleculeswithchemistryinliq = totaltimemoleculeswithchemistryinliq + float(entry[1])
            totalmassliquid = totalmassliquid + massM * float(entry[1])
            totalmasscheminliq = totalmasscheminliq + massA * float(entry[1])
            totaltimecheminliq = totaltimecheminliq + occurs * float(entry[1])
            for ii in range(len(population)):
                if population[ii][0] == entry[0] and population[ii][2] == 'L':   #cluster already exists in liquid
                    population[ii][iround+3] = entry[1]
                    flagcluster = 0
            if flagcluster == -1:
                population.append(['' for _ in range(len(population[0]))])
                population[len(population)-1][0] = entry[0]
                population[len(population)-1][1] = str(molec_size)
                population[len(population)-1][2] = 'L'
                population[len(population)-1][iround+3] = entry[1]
...
def main(argv):
    ...
    if rings == 0:
        list_of_files = glob.glob('./*.r0.stat.dat')  #$list_of_files = glob.glob('./*.r0.popul.dat')
    elif rings == 1:
        list_of_files = glob.glob('./*.r1.stat.dat')  #list_of_files = glob.glob('./*.r1.popul.dat')
    else:
    ...
    if len(list_of_files)>0:
        population = [['' for _ in range(len(list_of_files)+3)]]        #population = matrix containing all the clusters
                            # col 0: cluster name
                            # col 1: cluster size (number of atoms)
                            # col 2: V or L (vapor or liquid)
                            # col 3+: cluster time in each file
        ...
        for ifile in range(len(list_of_files)):
            ...
            population[0][ifile+3] = shortfile.split('stat.dat')[0].split('outcar')[0]
        for ifile in range(len(list_of_files)):
    ...
    #Print unified output file: all clusters (vapor + liquid), one row per cluster
    outfile = logfile + 'all.-t' + str(lifetime) + '.-L' + str(clusterlength) + '.-R' + str(rings) + '.-C=' + str(chemistry) + '.dat'

    ff = open(outfile,'w')
    # header: fixed columns then one column per input file, then cumulative columns
    string = 'Molecule\tSize\tV/L\t'
    for icol in range(len(list_of_files)):
        string = string + population[0][icol+3] + '\t'
    string = string + 'CumulativeTime\tCumulativePercent\t'
    string = string + '\n'
    ff.write(string)

    totalsimulationtime = totalsimulationvaportime + totalsimulationliquidtime
    for ipop in range(1,len(population)):
        ...
        # per-file times
        for icol in range(3, len(population[ipop])):
            if len(population[ipop][icol]) > 0:
                cumultime = cumultime + float(population[ipop][icol])
                string = string + str(population[ipop][icol]) + '\t'
            else:
                string = string + '\t'
        # cumulative time and percent over total simulation (V+L)
        string = string + str(cumultime) + '\t'
        if totalsimulationtime > 0:
            string = string + str(100*cumultime/totalsimulationtime) + '\t'
        else:
            string = string + '\t'
        string = string + '\n'
        ff.write(string)
