...
gofr_lib.compute_gofr.argtypes = (np.ctypeslib.ndpointer(dtype=np.int32, ndim=1,flags='C'),ctypes.POINTER(ctypes.c_double),ctypes.c_int,ctypes.POINTER(ctypes.c_int),ctypes.c_double,ctypes.POINTER(ctypes.c_double),ctypes.POINTER(ctypes.c_double),ctypes.c_int,ctypes.c_double,ctypes.c_int)
...
def compute_gofr_snapshot(MySnapshot,step,maxStep,Types,ntypes,discrete,acell,ndivx,ortho,maxlength):#Computes the distance matrix for each snapshot
    ...
    gofr_lib.compute_gofr(gofr,MSp,ntypes,Tp,discrete,acp,RVp,ndivx+1,maxlength,diag)
    #print("DIAG :", diag)
...
def main(argv):
    ...
    print('ndivx is ',ndivx)
        
    ntypes = len(Types)-2
    compute_gofr_red = partial(compute_gofr_snapshot,maxStep=length-1,Types=Types,ntypes=ntypes,discrete=discrete,acell=acell,ndivx=ndivx,ortho=ortho,maxlength=maxlength)
    ...
    gofrname = umdfile[:-8]+".gofr_new_notcubic.dat"
    fa = open(gofrname,'w')
    ...
    volcell = np.linalg.det(rprimd) #Surrounding volume that we took in account (for each atom) for the calculation of the gofr
