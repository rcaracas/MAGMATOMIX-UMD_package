...
__kernel void half_pdist(const INT M, const __global half* X,
                         __global half* res, __global half* coeff)
{
    // thread identifiers
    const INT globalRow = get_global_id(0); // row index of res in {0, ..., M-1}
    const INT globalCol = get_global_id(1); // col index of res in {0, ..., M-1}
    // 2d upper triangular indices mapped to 1d
    const INT k_index = M *(M-1)/2 - (M-globalRow)*((M-globalRow)-1)/2 +
                        globalCol - globalRow - 1;

    REAL acc = 0.0f, dist = 0.0f, k_coeff = 0.0f;
    int shift = 0;
...
__kernel void half_tiled_pdist(const INT M, const __global half* X,
                               __global half* res, __global half* coeff)
{    
    // thread identifiers
    const INT row = get_local_id(0); // local row index in {0, ..., TS-1}
    const INT col = get_local_id(1); // local col index in {0, ..., TS-1}
    const INT globalRow = TS * get_group_id(0) + row; // row index of X in {0, ..., M-1}
    const INT globalCol = TS * get_group_id(1) + col; // col index of X in {0, ..., M-1}
     // local memory to fill tiles of size TS * K
     __local REAL_IN Asub[TS][K];
     __local REAL_IN tAsub[TS][K];
...
__kernel void pdist(const INT M, const __global REAL* X,
                    __global REAL* res, __global REAL* coeff)
{    
    // thread identifiers
    const INT globalRow = get_global_id(0); // row index of res in {0, ..., M-1}
    const INT globalCol = get_global_id(1); // col index of res in {0, ..., M-1}
...
// local memory storage.
__kernel void tiled_pdist(const INT M, const __global REAL* X,
                          __global REAL* res, __global REAL_IN* coeff)
{    
    // thread identifiers
    const INT row = get_local_id(0); // local row index in {0, ..., TS-1}
    const INT col = get_local_id(1); // local col index in {0, ..., TS-1}
